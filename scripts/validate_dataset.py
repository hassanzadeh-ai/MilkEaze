...
should not silently persist into the large collection. ``--strict`` exits non-zero on
any error, which is the form to run once the schema is frozen.
"""
...
from milkeaze.data.schema import ERROR, validate_dataset
...
def main() -> int:
    ...
    try:
        reports = validate_dataset(Path(args.root))
    ...
    print(f"\n{len(reports)} capture(s), {n_errors} error(s)")
